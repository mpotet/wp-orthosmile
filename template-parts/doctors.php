...
if ( ! orthosmile_get_option( 'show_doctors', '1' ) ) return;

$section_title = orthosmile_get_option( 'equipe_title',      __( 'Notre équipe', 'orthosmile' ) );
$section_subtitle = orthosmile_get_option( 'equipe_subtitle', __( 'Des spécialistes passionnés à votre service', 'orthosmile' ) );
$team_img_url  = orthosmile_get_option( 'equipe_team_photo', '' );
$rdv_label     = orthosmile_get_option( 'equipe_rdv_label',  __( 'Prendre rendez-vous', 'orthosmile' ) );
...
$has_posts = $praticiens->have_posts();
...
<section class="team-section" id="team" aria-label="<?php esc_attr_e( 'Notre équipe', 'orthosmile' ); ?>">
	<div class="container">
		...
		<div class="orthosmile-empty-notice">
			...
			<p>
				<?php esc_html_e( 'Aucun praticien publié.', 'orthosmile' ); ?>
				<a href="<?php echo esc_url( admin_url( 'edit.php?post_type=praticien' ) ); ?>">
					<?php esc_html_e( 'Ajouter des praticiens →', 'orthosmile' ); ?>
				</a>
			</p>
		</div>
		<?php endif; ?>
